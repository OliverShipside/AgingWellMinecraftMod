...
public class GuidanceOverlay {
    ...
    public void onRenderOverlay(CustomizeGuiOverlayEvent.Chat event) {
        ...
        String hint   = "\u00a7oPress " + KeyBindings.TOGGLE_GUIDANCE.getTranslatedKeyMessage().getString()
                + " to hide  \u2022  /guidance next to skip";
        ...
        graphics.drawString(font, title,  tx, ty, TITLE_COL, false);
        ty += LINE_HEIGHT;
        graphics.drawString(font, goal,   tx, ty, GOAL_COL,  false);
        ty += LINE_HEIGHT;
        if (hasDetail) {
            graphics.drawString(font, detail, tx, ty, HINT_COL, false);
            ty += LINE_HEIGHT;
        }
        graphics.drawString(font, hint, tx, ty, HINT_COL, false);
    }
}