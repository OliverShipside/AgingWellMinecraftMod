...
public class BaritoneAdapter {
    ...
    public static void goToSpawn(Player player) {
        if (!checkBaritone(player)) return;
        try {
            var spawnPos = player.level().getLevelData().getRespawnData().pos();
            Object baritone = getPrimaryBaritone();
            Object goal = Class.forName("baritone.api.pathing.goals.GoalBlock")
                    .getConstructor(int.class, int.class, int.class)
    ...
    //    if (!baritoneAvailable) return "Baritone not installed \u2014 automation unavailable.";
    //    try {
    //        Object baritone = getPrimaryBaritone();
    //        Object pathingBehavior = baritone.getClass()
    //                .getMethod("getPathingBehavior").invoke(baritone);
    ...
    public static String getStatus(Player player) {
        return "Automation is not available in this version of AgingWell.";
    }
    ...
    private static boolean checkBaritone(Player player) {
        send(player, "\u00a7e[AgingWell] Automation is not available in this version due to a lack of baritone versions.");
        send(player, "\u00a77A separate version of this mod with full automation support is available for earlier versions of minecraft.");
        return false;
    }
